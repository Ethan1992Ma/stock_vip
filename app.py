                        ...
                        if "GEMINI_API_KEY" not in st.secrets:
                            ...
                            with st.spinner("正在連線 AI 大腦..."):
                                try:
                                    ...
                                    ## 核心準則
                                    1. **數據邊界**：僅依賴下方提供的【技術監控數據】，不夾帶外部新聞。
                                    2. **語氣規範**：使用「策略性語言」（如：「若...則...」），避免模稜兩可的預測。
                                    3. **機率思維**：區分「高機率順勢」與「低機率逆勢」情境。

                                    ---

                                    ## 輸入數據 (Input Data)
                                    **分析標的**：{ticker_input} ({info.get('longName', '')})
                                    **技術監控數據**：
                                    {tech_insight}
                                    ...
                                    ## 任務指令
                                    請根據上述數據，嚴格依照以下結構輸出分析報告（請使用繁體中文，並保持 Markdown 格式）：

                                    ### 1. 技術事實掃描 (Technical Facts)
                                    *列出純粹的觀察數據，包含：*
                                    - **價格與均線**：目前的均線排列狀態。
                                    - **K 線型態**：是否有明顯反轉或中繼型態。
                                    - **指標與量能**：RSI 位置、MACD 狀態、成交量變化。

                                    ### 2. 市場結構判定 (Market Structure)
                                    - **趨勢定義**：多頭 / 空頭 / 震盪。
                                    - **目前位階**：主趨勢發動中 / 修正波 / 反彈波。
                                    ...
                                    *請依據數據推算並填寫：*
                                    - **當前價格**：(請填寫)
                                    - **關鍵支撐 (Support)**：(數值與依據)
                                    - **關鍵壓力 (Resistance)**：(數值與依據)
                                    - **多空分界/結構破壞點**：(數值)

                                    ### 4. 走勢機率評估 (Probability Assessment)
                                    *基於當前結構，評估未來三種走勢：*
                                    - **高機率情境**：(方向與理由)
                                    - **中機率情境**：(方向與理由)
                                    - **低機率情境**：(方向與理由)

                                    ### 5. 交易交戰手冊 (Tactical Battle Manual)
                                    *針對關鍵價位提供具體操作條件（非投資建議，僅為策略推演）：*
                                    - **【戰術 A：支撐區攻防】**：若回測支撐，觀察什麼訊號（K線/量能）才視為止跌？失效條件為何？
                                    - **【戰術 B：壓力區攻防】**：若挑戰壓力，需要什麼樣的量能才算有效突破？
                                    - **【戰術 C：現價應對】**：針對目前價格，是該等待、觀望還是依託均線操作？
